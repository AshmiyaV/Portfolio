...
export default function SkillSet() {
    ...
    // Track drag velocity manually
    const velocity = useRef({ rotation: 0 });
    ...
    useGSAP(() => {
        ...
        const handleResize = () => {
            // Set up the motion path for each box
            gsap.set(boxes, {
                motionPath: {
        ...
        Draggable.create(`.${styles.circularCarousel}`, {
            ...
            onPress: () => {
                ...
                velocity.current = { rotation: 0 }; // Reset velocity
                lastTime.current = performance.now(); // Reset time for velocity calculation
            },
            onRelease: () => {
                setDragStatus(null);
            },
            ...
            onDrag: function() {
                ...
                if (lastTime.current) {
                    ...
                    const deltaRotation = this.rotation - (this.vars.rotation || 0);
                    velocity.current.rotation = deltaRotation / deltaTime; // rotation velocity
                }
                lastTime.current = currentTime;
            },
            onDragEnd: function() {
                // Apply inertia effect using calculated velocity
                const dragSpeed = velocity.current.rotation;
            
                // Reduce the multiplier for much less spin after drag
                const rotationAmount = dragSpeed * 15; // Even smaller multiplier to reduce spin
            
                // Optionally limit the rotationAmount to a very small range
                const maxRotation = 15; // Cap the rotation to 60 degrees at most
                const finalRotation = Math.min(Math.max(rotationAmount, -maxRotation), maxRotation); // Cap the rotation
            
                gsap.to(this.target, {
                    rotation: `+=${finalRotation}`,
                    duration: Math.abs(finalRotation) / 100, // Faster stop for quicker deceleration
                    ease: "elastic.out",
                    onComplete: () => {
    ...
    // useEffect(() => {
    //     const boxes = gsap.utils.toArray(`.${styles.box}`);
    //     const anglePerBox = 360 / boxes.length;

    //     const handleResize = () => {
    //         // Recalculate motion path for responsiveness
    //         gsap.set(boxes, {
    //             motionPath: {
    //                 path: "#circularCarouselPath",
    //                 align: "#circularCarouselPath",
    ...
    //         type: "rotation",
    //         inertia: true,
    //         snap: (endValue) => gsap.utils.snap(anglePerBox, endValue),
    //         onPress: () => {
    //             velocity.current = { rotation: 0 }; // Reset velocity
    //             lastTime.current = performance.now(); // Reset time for velocity calculation
    //         },
    //         onDrag: function () {
    //             // Track rotation velocity
    //             const currentTime = performance.now();
    //             if (lastTime.current) {
    //                 const deltaTime = currentTime - lastTime.current;
    //                 const deltaRotation = this.rotation - (this.vars.rotation || 0);
    //                 velocity.current.rotation = deltaRotation / deltaTime;
    //             }
    //             lastTime.current = currentTime;
    //         },
    //         onDragEnd: function () {
    //             const dragSpeed = velocity.current.rotation;
    //             const rotationAmount = dragSpeed * 10;
    //             const maxRotation = anglePerBox * 2;
    //             const finalRotation = Math.min(Math.max(rotationAmount, -maxRotation), maxRotation);
    ...
    //                 onUpdate: () => {
    //                     const currentRotation = gsap.getProperty(this.target, "rotation") % 360;
    //                     const adjustedRotation = (currentRotation + 360) % 360;
    //                     const closestIndex = Math.round(adjustedRotation / anglePerBox) % boxes.length;
